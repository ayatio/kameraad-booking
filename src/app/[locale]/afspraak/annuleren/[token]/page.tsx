...
import { getAppointmentByCancelToken, canModify } from '@/lib/services/manage'
...
export default async function AnnulerenPage({ params: { locale, token } }: Props) {
  const [detail, settingsRaw, t] = await Promise.all([
    getAppointmentByCancelToken(token),
    getSettings(),
  ...
  const alreadyCancelled = detail.appointment.status === 'cancelled'
  ...
  return (
    <section className="min-h-screen" style={{ background: '#0c0b0a' }}>
      <div
        ...
        style={{
          background:
            'radial-gradient(120% 80% at 20% 0%, rgba(201,162,75,.06) 0%, transparent 55%)',
        }}
        aria-hidden="true"
      />
          ...
          >
            {alreadyCancelled ? (
              <div className="flex flex-col gap-2">
                ...
                >
                  {t('cancel.alreadyCancelledHeading')}
                </p>
                <p className="text-smoke" style={{ fontSize: '0.9rem', lineHeight: 1.6 }}>
                  {t('cancel.alreadyCancelledBody')}
                </p>
              </div>
            ) : !modifiable ? (
              <div className="flex flex-col gap-2">
                <p
                  ...
                  style={{ fontSize: '1.1rem' }}
                >
                  {t('cancel.outsideWindowHeading')}
                </p>
                <p className="text-smoke" style={{ fontSize: '0.9rem', lineHeight: 1.7 }}>
                  {t('cancel.outsideWindowBody', { hours: windowHours, phone })}
                </p>
              </div>
            ) : (
